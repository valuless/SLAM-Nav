...
- `scripts/time_corrector.py`: timestamp correction for the TF and point-cloud path.
...
| Registered point cloud | `/cloud_registered_fixed` | FAST-LIO output after timestamp handling |
...
If timestamps do not align, inspect `robot_navigation_bringup/scripts/time_corrector.py` and the remappings in `robot_navigation_bringup/launch/rslidar_scan.launch.py`.

### Invalid or empty `/scan`

Check:

- Whether `/cloud_registered_fixed` is being published.
