...
├── dobot_atom_sdk/          # Dobot Atom SDK 相关文件
...
- `launch/bringup.launch.py`：系统级启动入口，组合雷达驱动、点云转换、FAST-LIO、激光扫描生成、Dobot 桥接和 Nav2。
...
1. 先看 `src/robot_navigation_bringup/launch/bringup.launch.py`，理解系统启动了哪些模块。
...
```bash
ros2 launch robot_navigation_bringup bringup.launch.py
```
...
当前 `bringup.launch.py` 中地图路径默认值带有现场部署特征，迁移机器或工作区后需要通过 launch 参数覆盖。
...
| 需求 | 优先查看 |
| --- | --- |
| 修改系统启动顺序 | `robot_navigation_bringup/launch/bringup.launch.py` |
...
| 修改地图路径 | `robot_navigation_bringup/launch/bringup.launch.py` |
...
- `bringup.launch.py` 中部分默认值带有现场部署特征，迁移环境时应优先通过 launch 参数覆盖。
- `src/src/` 中存在重复内容，构建前应确认是否会造成同名包冲突。
